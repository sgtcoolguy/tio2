...
Suite.prototype.runApp = function runApp(next) {
	...
	var resultsStartRegex = /\!TEST_RESULTS_START\!/,
		...
		finished,
		targetLib,
		finalize = function (err, simHandle) {
			if (finished) return;
			finished = true;
			targetLib && targetLib[target].stop(simHandle);
console.log(err);
console.log(simHandle);
			simHandle && simHandle.appDir && (this.appDir = simHandle.appDir);
			next();
		}.bind(this);
